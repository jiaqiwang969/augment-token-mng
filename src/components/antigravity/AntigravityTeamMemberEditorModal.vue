<template>
  <BaseModal
    :visible="true"
    :title="mode === 'create' ? $t('platform.antigravity.apiService.addMember') : $t('platform.antigravity.apiService.editMember')"
    :show-close="true"
    :body-scroll="true"
  ...
  >
    <div class="space-y-4">
      <div class="grid gap-4 md:grid-cols-2">
        <div class="space-y-2">
          <label class="label mb-0">{{ $t('platform.antigravity.apiService.memberNameLabel') }}</label>
          <input
            v-model="draft.name"
            class="input"
            :disabled="busy"
            :placeholder="$t('platform.antigravity.apiService.memberNamePlaceholder')"
          />
        </div>
        <div class="space-y-2">
          <label class="label mb-0">{{ $t('platform.antigravity.apiService.memberCodeLabel') }}</label>
          <input
            v-model="draft.memberCode"
            class="input font-mono"
            :disabled="busy"
            :placeholder="$t('platform.antigravity.apiService.memberCodePlaceholder')"
          />
        </div>
      </div>
      ...
      <div class="grid gap-4 md:grid-cols-[minmax(0,1fr)_180px]">
        <div class="space-y-2">
          <label class="label mb-0">{{ $t('platform.antigravity.apiService.roleTitleLabel') }}</label>
          <input
            v-model="draft.roleTitle"
            class="input"
            :disabled="busy"
            :placeholder="$t('platform.antigravity.apiService.roleTitlePlaceholder')"
          />
        </div>
        <div class="space-y-2">
          <label class="label mb-0">{{ $t('platform.antigravity.apiService.colorLabel') }}</label>
      ...
      <div class="rounded-lg border border-border p-3">
        <div class="flex items-center justify-between gap-3">
          <div>
            <div class="text-sm font-semibold text-text-primary">{{ $t('platform.antigravity.apiService.memberKey') }}</div>
            <p class="mt-1 text-xs text-text-muted">
              {{ mode === 'create'
                ? $t('platform.antigravity.apiService.autoGenerateApiKeyHint')
                : $t('platform.antigravity.apiService.keySuffixLabel', { suffix: keySuffix || '-' }) }}
            </p>
          </div>
          <label class="flex items-center gap-2 text-[12px] text-text-secondary">
            <input
              ...
              :disabled="busy"
            />
            <span>{{ draft.enabled ? $t('platform.antigravity.apiService.enabledKey') : $t('platform.antigravity.apiService.disabledKey') }}</span>
          </label>
        </div>
        <div class="mt-3 flex gap-2">
          <input
            ...
            class="input font-mono"
            :disabled="busy"
            :placeholder="$t('platform.antigravity.apiService.apiKeyPlaceholder')"
          />
          <button
            class="btn btn--icon btn--ghost !h-[34px] !w-[34px] shrink-0"
            :disabled="busy"
            :title="isKeyVisible ? $t('platform.antigravity.apiService.hideApiKey') : $t('platform.antigravity.apiService.showApiKey')"
      ...
      <div class="space-y-2">
        <label class="label mb-0">{{ $t('platform.antigravity.apiService.personaSummaryLabel') }}</label>
        <textarea
          v-model="draft.personaSummary"
          class="input min-h-[84px] resize-y"
          :disabled="busy"
          :placeholder="$t('platform.antigravity.apiService.personaSummaryPlaceholder')"
        ></textarea>
      </div>

      <div class="space-y-2">
        <label class="label mb-0">{{ $t('platform.antigravity.apiService.notesLabel') }}</label>
        <textarea
          v-model="draft.notes"
          class="input min-h-[84px] resize-y"
          :disabled="busy"
          :placeholder="$t('platform.antigravity.apiService.notesPlaceholder')"
        ></textarea>
      </div>
    ...
    <template #footer>
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="flex flex-wrap items-center gap-2">
          ...
          >
            {{ $t('platform.antigravity.apiService.copyAccessBundle') }}
          </button>
          <button
            v-if="mode === 'edit'"
            class="btn btn--ghost btn--sm"
          ...
          >
            {{ $t('platform.antigravity.apiService.generateApiKey') }}
          </button>
          <button
            v-if="mode === 'edit'"
...
defineEmits(['close', 'save', 'regenerate', 'delete', 'copy-access'])
