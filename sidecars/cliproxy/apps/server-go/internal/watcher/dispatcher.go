...
var loadAuggieSessionFile = sdkAuth.LoadAuggieSessionFile
...
func snapshotCoreAuths(cfg *config.Config, authDir string) []*coreauth.Auth {
	...
	return synthesizeAuggieSessionAuth(ctx, out)
}

func synthesizeAuggieSessionAuth(ctx *synthesizer.SynthesisContext, auths []*coreauth.Auth) []*coreauth.Auth {
	if ctx == nil {
		return auths
	}

	session, err := loadAuggieSessionFile()
	if err != nil || session == nil {
		return auths
	}

	sessionAuth, err := sdkAuth.ApplyAuggieSession(nil, session)
	if err != nil || sessionAuth == nil {
		return auths
	}

	matchedExisting := false
	for idx, existing := range auths {
		if !sameAuggieSnapshotAuth(existing, sessionAuth) {
			continue
		}
		base := prepareAuggieSnapshotMergeBase(existing)
		updated, errApply := sdkAuth.ApplyAuggieSession(base, session)
		if errApply != nil || updated == nil {
			return auths
		}
		auths[idx] = updated
	...
	if sessionAuth.Attributes == nil {
		sessionAuth.Attributes = make(map[string]string)
	}
	sessionAuth.Attributes["runtime_only"] = "true"
	sessionAuth.Attributes["source"] = "session:auggie"
	synthesizer.ApplyAuthExcludedModelsMeta(sessionAuth, ctx.Config, nil, "oauth")
	if sessionAuth.CreatedAt.IsZero() {
		sessionAuth.CreatedAt = ctx.Now
	}
	if sessionAuth.UpdatedAt.IsZero() {
		sessionAuth.UpdatedAt = ctx.Now
	}
	return append(auths, sessionAuth)
}

func sameAuggieSnapshotAuth(existing, sessionAuth *coreauth.Auth) bool {
	if existing == nil || sessionAuth == nil {
		return false
	}
	if !strings.EqualFold(strings.TrimSpace(existing.Provider), "auggie") {
		return false
	}
	if existing.ID != "" && sessionAuth.ID != "" && strings.EqualFold(strings.TrimSpace(existing.ID), strings.TrimSpace(sessionAuth.ID)) {
		return true
	}
	existingTenant := snapshotAuggieTenant(existing)
	sessionTenant := snapshotAuggieTenant(sessionAuth)
	return existingTenant != "" && existingTenant == sessionTenant
}

func snapshotAuggieTenant(auth *coreauth.Auth) string {
	...
	return normalizedTenantURL
}

func prepareAuggieSnapshotMergeBase(auth *coreauth.Auth) *coreauth.Auth {
	if auth == nil {
		return nil
	}
	base := auth.Clone()
	...
	if label := strings.TrimSpace(base.Label); label != "" && !strings.EqualFold(label, "auggie") {
		return base
	}
	if storedLabel, _ := base.Metadata["label"].(string); strings.TrimSpace(storedLabel) != "" {
		base.Label = strings.TrimSpace(storedLabel)
	}
	return base
}
